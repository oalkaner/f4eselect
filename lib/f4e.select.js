(function($, window, document){
    ...
    var messages = {
        typetoadd : 'Type to add new item'
    };
    ...
    var templates = {
            ...
            button : {
                button : Handlebars.compile(
                    '{{#if wraptext}}' +
                    '<a class=\'f4e-button\' style=\'width:{{width}}vw;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;\'>{{text}}</a>' + 
                    '{{else}}'+ 
                    '<a class=\'f4e-button\' style=\'width:{{width}}vw\'>{{text}}</a>' + 
                    '{{/if}}'
                ),
                
                link : Handlebars.compile(
                    '{{#if wraptext}}' +
                    '<a class=\'f4e-button-link\' style=\'width:{{width}}vw;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;\'>{{text}}</a>' + 
                    '{{else}}'+ 
                    '<a class=\'f4e-button-link\' style=\'width:{{width}}vw;\'>{{text}}</a>' + 
                    '{{/if}}'
                ),
                select : Handlebars.compile(
                    '{{#if wraptext}}' +
                    '<div class=\'f4e-button-select\' style=\'width:{{width}}vw;\'><div style=\'width:{{selectwidth}}vw;float:left;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;\'>{{text}}</div></div>' + 
                    '{{else}}'+ 
                    '<div class=\'f4e-button-select\' style=\'width:{{width}}vw;\'><div style=\'float:left;width:{{selectwidth}}vw;\'>{{text}}</div></div>' + 
                    '{{/if}}'
                )
            },
            ...
            menuItems : Handlebars.compile('<div class=\'f4e-menu-items\'' + 'style=\'max-height:{{menuItemsMaxHeight}}vh;overflow:auto\'>' + 
                                              ...
                                              '{{else}}' +
                                                '{{#each options}}'  +
                                                      ...
                                                      '{{else}}' + 
                                                            '{{#if isDisabled}}' + 
                                                                '<div class=\'f4e-menu-item disabled\' value=\'{{val}}\'>' + 
                                                            '{{else}}' +
                                                                '<div class=\'f4e-menu-item\' value=\'{{val}}\'>' + 
                                                            '{{/if}}' +
    ...
    dropdown.prototype = {
        ...
        init : function(){     
            ...
            var context = {
                text : this.settings.selecttext,
                wraptext : this.settings.wrapbuttontext,
                width : this.settings.width,
                selectwidth : this.settings.width * 0.85
            }
            
            this.$dropDown = $('<div class=\'f4e-dropdown\'>' + templates.button[this.settings.buttontype](context) + '</div>');            
        ...
        bindEvents : function(){
            ...
            this.$dropDown.on('click',function(e){
             ...
             this.$dropDown.on('click', '[class*=\'f4e-button\']',function(e){
            ...
            this.$dropDown.on('click', '.f4e-toolbar .fa-check', function(e){
                _this.selectAllItemsCallback(e);
            });        
                        
            this.$dropDown.on('click', '.f4e-toolbar .fa-remove', function(e){
                _this.clickDeselectAllCallback(e);
            }); 
            
            this.$dropDown.on('click', '.f4e-toolbar .fa-sort', function(e){
        ...
        clickAddNewOptionCallback : function(e){
            ...
            if(newItem != ''){
                ...
                this.$dropDown.find('.searchbox').attr('placeholder', messages.typetoadd);
            }
        },
        ...
        renderMenu : function(){
            var text = '<div class=\'f4e-menu\' style=\'width:' + this.settings.width + 'vw\'>';
            text += '<div>'
        ...
        renderMenuItems : function(searchtext, ingest){
            ...
            $options.each(function(index){
                ...
                var disabled = $(this).get(0).hasAttribute('data-disabled');
                var data_content = $(this).attr('data-content');
                var data_subtext = $(this).attr('data-subtext');
                var isDivider = $(this).get(0).hasAttribute('data-divider');
                ...
                context.options.push({
                   ...
                   isDataDividerEnabled : isDivider,
        ...
        setButtonText : function(){
            ...
            if(this.settings.buttontype == 'select'){
                this.$dropDown.find('[class*=\'f4e-button\'] div').text(text);
            }            
            else
                this.$dropDown.find('[class*=\'f4e-button\']').text(text);
            
        },
        
        displayMenu : function(){            
            var $menu = $(this.renderMenu());
            this.$dropDown.find('[class*=\'f4e-button\']').next().remove();
            this.$dropDown.find('[class*=\'f4e-button\']').after($menu);
